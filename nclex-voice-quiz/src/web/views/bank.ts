...
import { button, clear, debounce, el, questionDetails, safeCategoryName, select, sourceLabel, statusBox } from "../ui.js";
...
export function bankView(root: HTMLElement): Dispose {
  ...
  view.append(
    el(
      ...
      el("div", { class: "row" }, importLabel, exportButton, exportLink, el("span", { class: "muted small" }, "Import accepts ", el("code", { text: "{ version: 1, questions: [...] }" }), " (see docs/IMPORT_FORMAT.md).")),
      status.node,
    ),
  ...
  function renderPage(): void {
    ...
    for (const question of slice) {
      ...
      list.appendChild(questionDetails(question, { actions, testid: `bank-question-${question.id}` }));
    }
    shown += slice.length;
  ...
  async function importFile(file: File): Promise<void> {
    ...
    try {
      ...
      if (!Array.isArray(questions)) throw new Error('Expected { "version": 1, "questions": [...] } or an array of questions.');
      const report = await api.importQuestions({ version: 1, questions: questions as Question[] });
      if (disposed) return;
